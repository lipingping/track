$(function () {
	...
	// Check or uncheck all checkboxes ,used at instances/tabs.html:6L
	$('.check_all').click(function() {
		...
		if(selected == true){
			...
			$('button[value="terminate_instance"]').removeClass('disabled');
		}
		else{
			$('button[value="soft_reboot"]').addClass('disabled');
			$('button[value="terminate_instance"]').addClass('disabled');
		}

	});
	// Instances tr selector , used at instances/tabs.html:19L,
	$('.instance_chooser').click(function() {
	...
	// Instances action button ,used at instances/_index.html:9-11L
	$('button[data-toggle="button"]').click(function(){
			...
			var instance_id = tr.attr('instance_id');
			if(instance_id==null)
				return;
			var url = form.attr('action') + instance_id + '/'+ action_type; 

			var status_td = tr.find('td[id="instance_status"]');
	...
	//$(this).attr('class','active btn btn-warning');
		...
		var id = href.split('=')[1]
		$("#instance_detail_tab li a[data-target='" + id + "']").parents('li').addClass('active');

		var req = curr_url + href;
		$("#instanceTabContent").load(req +" #"+id);

		//$.get(curr_url+'/'+href,null,function(result){
		//	//$(".tab-content").html("");
		//});
    })
